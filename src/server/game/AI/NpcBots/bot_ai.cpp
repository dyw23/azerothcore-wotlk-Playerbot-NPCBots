...
bool bot_ai::OnGossipSelect(Player* player, Creature* creature/* == me*/, uint32 sender, uint32 action)
{
    ...
    switch (sender)
    {
        ...
        case GOSSIP_SENDER_DEBUG_ACTION:
        {
            ...
            switch (action - GOSSIP_ACTION_INFO_DEF)
            {
                ...
                case 4: //list roles
                {
                    ...
                    for (uint32 i = BOT_MAX_ROLE; i != BOT_ROLE_NONE; i >>= 1)
                    {
                        if (_roleMask & i)
                        {
                            ...
                            {
                                case BOT_ROLE_TANK:
                                    ch.SendSysMessage("主坦克");
                                    break;
                                case BOT_ROLE_TANK_OFF:
                                    ch.SendSysMessage("副坦克");
                                    break;
                                case BOT_ROLE_DPS:
                                    ch.SendSysMessage("DPS");
                                    break;
                                case BOT_ROLE_HEAL:
                                ...
                                //    break;
                                case BOT_ROLE_RANGED:
                                    ch.SendSysMessage("远程职业");
                                    break;
                                case BOT_ROLE_PARTY:
                                    ch.SendSysMessage("辅助职业");
                                    break;
                                default:
                ...
                case 5: //list spells
                {
                    close = false;
                    ChatHandler ch(player->GetSession());
                    ch.PSendSysMessage("{}的技能:", me->GetName());
                    uint32 counter = 0;
                    SpellInfo const* spellInfo;
                    BotSpellMap const& myspells = GetSpellMap();
                    for (BotSpellMap::const_iterator itr = myspells.begin(); itr != myspells.end(); ++itr)
                    {
                        ...
                        sstr << " 编号: " <<  itr->second->spellId << ", 基础值: " << itr->first
                            << ", 冷却时间: " << itr->second->cooldown << ", 基础值: " << std::max<uint32>(spellInfo->RecoveryTime, spellInfo->CategoryRecoveryTime);
                        if (itr->second->enabled == false)
                            sstr << " (disabled)";
                ...
                case 6: //reload config
                {
                    ...
                    BotMgr::ReloadConfig();
                    ch.SendGlobalGMSysMessage("NPC机器人配置设置已重新加载。");
        ...
        case GOSSIP_SENDER_DEBUG:
        {
            ...
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, "<重置统计数据>", GOSSIP_SENDER_DEBUG_ACTION, GOSSIP_ACTION_INFO_DEF + 2);
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, "<列出统计数据>", GOSSIP_SENDER_DEBUG_ACTION, GOSSIP_ACTION_INFO_DEF + 3);
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, "<列出职责>", GOSSIP_SENDER_DEBUG_ACTION, GOSSIP_ACTION_INFO_DEF + 4);
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, "<列出技能>", GOSSIP_SENDER_DEBUG_ACTION, GOSSIP_ACTION_INFO_DEF + 5);
